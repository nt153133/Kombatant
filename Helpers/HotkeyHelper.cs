...
namespace Kombatant.Helpers
{
	...
	internal class HotkeyHelper
	{
		...
		{
			if (Hotkeys.Instance.EnablePauseKey)
				RegisterPauseToggleHotkey();
			if (Hotkeys.Instance.EnableAutoFaceKey)
				RegisterAutoFaceTargetToggleHotkey();
			if (Hotkeys.Instance.EnableAutonomousKey)
				RegisterAutonomousToggleHotkey();
			if (Hotkeys.Instance.EnableAutoTargetKey)
				RegisterAutoTargetToggleHotkey();
			if (Hotkeys.Instance.EnableAvoidanceKey)
				RegisterAvoidanceToggleHotkey();
			if (Hotkeys.Instance.EnableFollowingKey)
				RegisterFollowingToggleHotkey();
		...
		internal void UnregisterHotkeys()
		{
			...
			HotkeyManager.Unregister(Hotkey.HotkeyFollowingToggle);
		...
		/// Unregisters/Registers hotkeys.
		/// </summary>
		internal void ReloadNonDynamicHotkeys()
		{
			HotkeyManager.Unregister(Hotkey.HotkeyPauseToggle);
			HotkeyManager.Unregister(Hotkey.HotkeyAutoFaceToggle);
			HotkeyManager.Unregister(Hotkey.HotkeyAutonomousToggle);
		...
		/// <summary>
		/// Registers the "Following" hotkey.
		/// </summary>
		private void RegisterFollowingToggleHotkey()
		{
			HotkeyManager.Register(Hotkey.HotkeyFollowingToggle, Hotkeys.Instance.FollowingToggleKey, Hotkeys.Instance.FollowingToggleModifierKey, hk =>
			{
				BotBase.Instance.EnableFollowing = !BotBase.Instance.EnableFollowing;
